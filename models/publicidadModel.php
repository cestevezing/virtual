...
class PublicidadModel extends Model
{
    ...
    {
        try {
            $query = $this->db->connect()->prepare('INSERT INTO tb_publicidad ( pu_nombre_empresa, pu_lema, pu_imagen, pu_link) VALUES (:nombre,:lema,:imagen,:link)');
            $query->execute([
                'nombre' => $datos['nombre'],
                'lema' => $datos['lema'],
                'imagen' => $datos['imagen'],
    ...
    function listarCuatro()
    {
        try {
            $items = [];
            $query = $this->db->connect()->query('SELECT * FROM tb_publicidad ORDER BY rand() LIMIT 4');
            while ($row = $query->fetch()) {
                $item = new Publicidad();
                $item->id = $row['pu_id'];
                $item->nombre = $row['pu_nombre_empresa'];
